...
export class colorService {
  ...
  colorConverter(color) {
    ...
    return this.attachValues(this.toColorObject(color));
  }
  toRgbString() {
  ...
  ncsToRgb(ncs) {
    var black,
      ...
      g,
      b;
    ncs = this.w3trim(ncs).toUpperCase();
    ncs = ncs.replace('(', '');
    ncs = ncs.replace(')', '');
  ...
  roundDecimals(c) {
    ...
    c.hue = c.hue ? Number(c.hue.toFixed(0)) : 0;
    c.sat = c.sat ? Number(c.sat.toFixed(2)) : 0;
    c.lightness = c.lightness ? Number(c.lightness.toFixed(2)) : 0;
    c.whiteness = c.whiteness ? Number(c.whiteness.toFixed(2)) : 0;
    c.blackness = c.blackness ? Number(c.blackness.toFixed(2)) : 0;
    c.cyan = c.cyan ? Number(c.cyan.toFixed(2)) : 0;
    c.magenta = c.magenta ? Number(c.magenta.toFixed(2)) : 0;
    c.yellow = c.yellow ? Number(c.yellow.toFixed(2)) : 0;
    c.black = c.black ? Number(c.black.toFixed(2)) : 0;
    c.ncol = c.ncol
      ? c.ncol.substr(0, 1) + Math.round(Number(c.ncol.substr(1)))
      : '';
